...
from estimation_forecast_functions_var import FIGARCH_Forecaster_1_d_0_MLE_delta_stds, GARCH_Forecaster_MLE_1_1_delta_stds, pre_whitening, GARCH_Forecaster_MLE_p_1_q_2_delta_stds, BMSM_Forecaster_MLE_delta_stds
...
def get_regression_features_MSM(train_forecasts, train_implied_vol_smile, stds_of_vols_train, train_true_vols, \
                                     Pi_train, backtest_forecasts, backtest_implied_vol_smile, stds_of_vols_backtest, \
                                     Pi_backtest, bit_states, m0):

    # first get MSM specific state values
    P_train = np.zeros((bit_states.shape[1], len(train_forecasts)))
    P_test = np.zeros((bit_states.shape[1], len(backtest_forecasts)))

    for i in range(len(train_forecasts)):
         placeholder = Pi_train[i] @ bit_states
         P_train[:, i] = placeholder * m0 + (1 - placeholder) * (2 - m0)
        
    for j in range(len(backtest_forecasts)):
         placeholder = Pi_backtest[j] @ bit_states
         P_test[:, j] = placeholder * m0 + (1 - placeholder) * (2 - m0)

    slow_train = np.mean(P_train[:4, :], axis = 0)
    fast_train = np.mean(P_train[4:, :], axis = 0)

    slow_backtest = np.mean(P_test[:4, :], axis = 0)
    fast_backtest = np.mean(P_test[4:, :], axis = 0)

    # continue with features identified before
    x1 = np.log(stds_of_vols_train)
    x2 = slow_train - fast_train

    x3 = train_implied_vol_smile['SKEW_ATM'].values
    x4 = train_implied_vol_smile['KURT'].values

    X = np.column_stack((x1, x2, x3[:len(x1)], x4[:len(x1)]))

    x1_t = np.log(stds_of_vols_backtest)
    x2_t = slow_backtest - fast_backtest

    x3_t = backtest_implied_vol_smile['SKEW_ATM'].values
    x4_t = backtest_implied_vol_smile['KURT'].values

    X_t = np.column_stack((x1_t, x2_t, x3_t[:len(x1_t)], x4_t[:len(x1_t)]))

    # get target values ready as well
    y_log = np.log(np.clip(train_true_vols, 1e-6, None))
    m_log = np.log(train_forecasts)
    b = y_log - m_log

    m_log_t = np.log(backtest_forecasts)

    return X, X_t, b, y_log, m_log, m_log_t

def get_regression_features_OTHER(train_forecasts, train_implied_vol_smile, stds_of_vols_train, train_true_vols, \
                                     backtest_forecasts, backtest_implied_vol_smile, stds_of_vols_backtest):

    # continue with features identified before
    x1 = np.log(stds_of_vols_train)

    x2 = train_implied_vol_smile['SKEW_ATM'].values
    x3 = train_implied_vol_smile['KURT'].values

    X = np.column_stack((x1, x2[:len(x1)], x3[:len(x1)]))

    x1_t = np.log(stds_of_vols_backtest)
    x2_t = backtest_implied_vol_smile['SKEW_ATM'].values
    x3_t = backtest_implied_vol_smile['KURT'].values

    X_t = np.column_stack((x1_t, x2_t[:len(x1_t)], x3_t[:len(x1_t)]))
...
def regression_forecasts_train(X, b, y_log, m_log, m_log_t, X_t):
    '''
    ...
    b is target here and ylog is what we construct after the regression. 
    '''
    ...
    L = int(np.floor(4 * (len(b) / 100) ** (2 / 9)))

    model = sm.OLS(b, X).fit(cov_type= 'HAC', cov_kwds={'maxlags': L})

    bhat_log_train = model.predict(X)
    resid_log = y_log - (m_log + bhat_log_train)
    smear = float(np.mean(np.exp(resid_log)))

    Xt = scalar.transform(X_t)
    Xt = sm.add_constant(Xt)

    bhat_log = model.predict(Xt)
    yhat_log = bhat_log + m_log_t
    vol_pred = np.exp(yhat_log) * smear

    return vol_pred, smear, model, scalar
...
def grid_search_params(backtest_data, param_grid, init_cap, OPTION_MATURITY, SIGNAL_LB, SIGNAL_UB, 
                       ...
                       verbose, convert_USD, OLS, MSM_data, MSM):
    ...
    train_implied_vol_smile_ols = backtest_data[10]
    train_true_vols_ols = backtest_data[11]
    train_stds_of_vols_ols = backtest_data[12]
    test_stds_of_vols = backtest_data[13]
    ...
    n_trials = int(param_grid.get('n_trials', 400)) # 400
    seed = int(param_grid.get('seed', 12))
    n_jobs = int(param_grid.get('n_jobs', 1))  

    # if OLS get features for regresssion before hyperparameter loop
    if OLS and MSM:

        Pi_train = MSM_data[0]
        Pi_test = MSM_data[1]
        bit_states = MSM_data[2]
        m0 = MSM_data[3]

        X, X_t, b, y_log, m_log, m_log_t = get_regression_features_MSM(
            train_forecasts_ols, train_implied_vol_smile_ols, train_stds_of_vols_ols, train_true_vols_ols, \
                Pi_train, forecasts, test_vol_smile, test_stds_of_vols, Pi_test, bit_states, m0)
        
        # no hyperparam just backtest using our OLS
        fc_use, _, _, _ = regression_forecasts_train(X, b, y_log, m_log, m_log_t, X_t)

    elif OLS and not MSM:

        X, X_t, b, y_log, m_log, m_log_t = get_regression_features_OTHER(
            train_forecasts_ols, train_implied_vol_smile_ols, train_stds_of_vols_ols, train_true_vols_ols, \
                forecasts, test_vol_smile, test_stds_of_vols)
    ...
    def run_once(long_thresh, short_thresh, signal_multiplier, fc_use):
    ...
    def objective(trial: optuna.Trial):
        ...
        sharpe, max_dd, calmar, cagr = \
                    run_once(long_thresh, short_thresh, signal_multiplier, fc_use)
...
def run_strategy_with_params(forecasts, test_implied_vol, test_vol_smile, r_base, r_term, 
    ...
    cash, trading_portfolio_value, total_portfolio_value, trading_summary, option_delta_positions, delta_hedge, delta_hedge_cashflows, hedge_carry, daily_option_pnl_real, daily_option_pnl_mtm, daily_hedge_pnl_mtm, vega, gamma = strategy.run_strategy()
    ...
    precision_all, precision_active, win_loss_ratio_all, win_loss_ratio_active, avg_win_all, avg_win_active, avg_loss_all, avg_loss_active, non_active_share, total_trades, vega, gamma
...
class Hedging_Trading_Strategy:
    ...
    def __init__(self, 
        ...
        self.vega = np.zeros(N + 1)
        self.vega[0] = 0.0

        self.gammas = np.zeros(N + 1)
        self.gammas[0] = 0.0
    ...
    def get_option_value(self, t,  vol, spot, r_b, r_t, K, type='call'):
        ...
        if self.length_of_option - t <= 0.0:
            if type == 'call':
    ...
    def get_vega(self, t,  vol, spot, r_b, r_t, K):
        ...
        N_diff = (1 / np.sqrt(2 * np.pi)) * np.exp(-0.5 * d1**2)

        vega = spot * np.sqrt(self.length_of_option - t) * N_diff * np.exp(-r_b * (self.length_of_option - t))

        vega_straddle = 2 * vega

        return vega_straddle

    def get_gamma(self, t, vol, spot, r_b, r_t, K):
        ...
        gamma = (np.exp(-r_b * (self.length_of_option - t)) * N_diff / (vol * spot * np.sqrt(self.length_of_option - t)))

        gamma_straddle = 2 * gamma

        return gamma_straddle
    ...
    def open_long_position(self, idx, spot_t, overnight_dom_r_t, tilde_vol_t, r_b_t, r_t_t, signal_strength_t):
        ...
        vega = self.get_vega(0.0, tilde_vol_t, spot_t, r_b_t, r_t_t, K)
        vega_adjusted = vega * notional_adjusted
        self.vega[idx] = vega_adjusted

        gamma = self.get_gamma(0.0, tilde_vol_t, spot_t, r_b_t, r_t_t, K)
        gamma_adjusted = gamma * notional_adjusted
        self.gammas[idx] = gamma_adjusted
    ...
    def get_vol_smile_strikes(self, vol_smile_t, spot_t, r_b_t, r_t_t):
        ...
        cleaned_smile_t = vol_smile_t.drop(['KURT', 'SKEW_ATM'])

        deltas = pd.to_numeric(cleaned_smile_t.index, errors='coerce')
        strikes = np.zeros_like(deltas)
        tau = self.length_of_option

        for i, delta in enumerate(deltas):

            vol = max(1e-8, cleaned_smile_t.iloc[i])
    ...
    def mark_to_market_long_position(self, idx, vol_smile_t, spot_t, r_b_t, r_t_t, remain_option_duration):
        ...
        t_elapsed = self.length_of_option - remain_option_duration
        vega = self.get_vega(t_elapsed, tilde_vol_from_smile, spot_t, r_b_t, r_t_t, last_strike)
        vega_adjusted = vega * last_notional_adjusted
        self.vega[idx] = vega_adjusted

        gamma = self.get_gamma(t_elapsed, tilde_vol_from_smile, spot_t, r_b_t, r_t_t, last_strike)
        gamma_adjusted = gamma * last_notional_adjusted
        self.gammas[idx] = gamma_adjusted
    ...
    def run_strategy(self):
        ...
        for t in range(1, N+1):
            ...
            long_condition = forecast_t >= self.long_thresh * (tilde_vol_t ** 2) # SINCE forecast is in variance decimal terms so convert implied vol to imlpied variance which is already in decimal terms
            short_condition = forecast_t <= self.short_thresh * (tilde_vol_t ** 2)
        ...
        if self.convert_USD:
            self.convert_to_USD()
            return self.MM_V_USD, self.trading_V_USD, self.V_USD, self.trading_summary, self.option_delta_position, self.total_hedge_position, self.adjustment_cashflow_USD, self.hedge_carry_USD, self.daily_option_pnl_real_USD, self.daily_option_pnl_mtm_USD, self.daily_hedge_pnl_mtm_USD, self.vega, self.gammas
        else:
            return self.MM_V, self.trading_V, self.V, self.trading_summary, self.option_delta_position, self.total_hedge_position, self.adjustment_cashflow, self.hedge_carry, self.daily_option_pnl_real, self.daily_option_pnl_mtm, self.daily_hedge_pnl_mtm, self.vega, self.gammas
    ...
    def performance_summary(self):
        ...
        # get log returns from the portfolio as these are additive so can be used in a combination portfolio
        returns_V = np.log(V[1:] / V[:-1])

        # for risk free rate always use domestic rate DO NOT USE USD unless this is term currency
        risk_free_rate = self.overnight_domestic_rate.values
        risk_free_rate_logs = np.log(1 + risk_free_rate) # log compounded for log returns
        excess_returns = returns_V - risk_free_rate_logs
...
class Compare_Trading_Strategies:
        ...
        def prepare_universal_series(self):
                ...
                self.train_realised_variance_oos = self.realised_variance_series.values[1:self.N//2 + int(self.N_test * self.train_size)]
                ...
                self.mid_vol_smile_train = self.vol_smile_data[:self.N//2] / 100
                self.mid_vol_smile_test = self.vol_smile_data[self.N//2:-self.H] / 100
                self.mid_vol_smile = self.vol_smile_data / 100
        ...
        def get_BMSM_data(self):
                ...
                norm_mse, norm_mae, R2, realised_var_H_forecasts, realised_var_H_true, stds_of_vars, realised_var_H_forecasts_train, realised_var_H_true_train, stds_of_vars_train, naive_forecast = bmsm_forecasts_backtest.compute_forecasts()
                self.Pi_train_backtest, self.Pi_test_backtest, self.bit_states_backtest, self.m0_backtest, self.log_L_bmsm, self.se_bmsm, self.alpha_p_bmsm, self.beta_p_bmsm, self.alpha_hat_bmsm, self.beta_hat_bmsm = bmsm_forecasts_backtest.access_extra_values()

                if self.verbose:
                        print(f"\nBMSM backtest estimation results:")
                        print(f"Norm MSE: {norm_mse}, Norm MAE: {norm_mae}, R2: {R2}")

                # collect decimal data for MSE/MAE analysis
                self.forecasts_mse_mae_bmsm = realised_var_H_forecasts[1:] / (100**2)
                self.true_var_series = realised_var_H_true[1:] / (100 ** 2)

                # collect data for strategy analysis
                # get decimal forecasts that line up with implied vols (1: so signal is based on our forecast for today vs implied vol yday)
                realised_var_forecasts = realised_var_H_forecasts[1:] / (100**2)
                self.forecasts_bmsm_backtest = realised_var_forecasts[:int(self.N_test * self.train_size)]

                stds_vars = stds_of_vars[1:]
                stds_vars_backtest = stds_vars[:int(self.N_test * self.train_size)]
                signal_backtest = 1 / stds_vars_backtest
                self.signal_bmsm_backtest = (signal_backtest - signal_backtest.min()) / (signal_backtest.max() - signal_backtest.min())

                self.forecasts_train_ols_bmsm_backtest = realised_var_H_forecasts_train / (100**2)
                self.true_var_series_train_ols_bmsm_backtest = realised_var_H_true_train / (100**2)

                self.stds_of_vars_bmsm_backtest_train = stds_of_vars_train
                self.stds_of_vars_bmsm_backtest = stds_vars_backtest
                self.stds_vars_mse_mae_bmsm = stds_vars
                self.naive_forecast_bmsm_backtest = naive_forecast
                ...
                norm_mse, norm_mae, R2, realised_var_H_forecasts, realised_var_H_true, stds_of_vars, realised_var_H_forecasts_train, realised_var_H_true_train, stds_of_vars_train, naive_forecast = bmsm_forecasts_oos.compute_forecasts()
                self.Pi_train_oos, self.Pi_test_oos, self.bit_states_oos, self.m0_oos, _, _, _, _, _, _ = bmsm_forecasts_oos.access_extra_values()
                ...
                self.forecasts_bmsm_oos = realised_var_H_forecasts[1:] / (100**2)
                signal_oos = 1 / stds_of_vars[1:]  
                ...
                self.forecasts_train_ols_bmsm_oos = realised_var_H_forecasts_train / (100**2)
                self.true_var_series_train_ols_bmsm_oos = realised_var_H_true_train / (100**2)

                self.stds_of_vars_bmsm_oos_train = stds_of_vars_train
                self.stds_of_vars_bmsm_oos = stds_of_vars[1:]
                self.naive_forecast_bmsm_oos = naive_forecast

        def get_hyperparameters(self, forecasts_backtest, signal_backtest, forecasts_train_ols, true_train_ols, train_stds, test_stds, MSM_indicator, OLS_bool):
                ...
                backtest_data = [forecasts_backtest, 
                 ...
                 forecasts_train_ols, 
                 self.mid_vol_smile_train.iloc[:len(forecasts_train_ols)], 
                 true_train_ols, 
                 train_stds, 
                 test_stds]
                ...
                if MSM_indicator:
                     MSM_data = [self.Pi_train_backtest, self.Pi_test_backtest, self.bit_states_backtest, self.m0_backtest]
                else:
                     MSM_data = None
                ...
                cv_results = grid_search_params(backtest_data, param_grid, self.initial_capital_domestic, self.option_maturity, self.signal_lb, self.signal_ub, 
                                                self.notional_base, self.maximum_delta_difference, self.transaction_cost_indicator, self.transaction_costs_spot,\
                                                self.transaction_costs_option, verbose = False, convert_USD=self.convert_USD, OLS = OLS_bool, MSM_data=MSM_data, \
                                                MSM=MSM_indicator)
                ...
                return long_optimal, short_optimal, sig_mult
        ...
        def run_trading_strategy(self, forecasts, signal, long_optimal, short_optimal, sig_mult, type, model, plots, verbose):
                ...
                if type == 'Backtest':
                        ...
                        avg_loss_all_bt, avg_loss_active_bt, non_active_share_bt, total_trades_bt, vega_bt, gamma_bt = run_strategy_with_params(
                                forecasts=forecasts,
                        ...
                        calmar_bt, cagr_bt, cash_no_trades, precision_all_bt, precision_active_bt, \
                        win_loss_ratio_all_bt, win_loss_ratio_active_bt, avg_win_all_bt, avg_win_active_bt, \
                        avg_loss_all_bt, avg_loss_active_bt, non_active_share_bt, total_trades_bt, vega_bt, gamma_bt
                        ...
                        avg_loss_all_oos, avg_loss_active_oos, non_active_share_oos, total_trades_oos, vega_oos, gamma_oos = run_strategy_with_params(
                                forecasts=forecasts,
                        ...
                        calmar_oos, cagr_oos, cash_no_trades, precision_all_oos, precision_active_oos, \
                        win_loss_ratio_all_oos, win_loss_ratio_active_oos, avg_win_all_oos, avg_win_active_oos, \
                        avg_loss_all_oos, avg_loss_active_oos, non_active_share_oos, total_trades_oos, vega_oos, gamma_oos
        ...
        def get_BMSM_hyperparams_and_run_strategy(self, verbose):
                ...
                self.long_optimal_bmsm, self.short_optimal_bmsm, self.sig_mult_bmsm = \
                        self.get_hyperparameters(self.forecasts_bmsm_backtest, self.signal_bmsm_backtest, self.forecasts_train_ols_bmsm_backtest, \
                                                 self.true_var_series_train_ols_bmsm_backtest, self.stds_of_vars_bmsm_backtest_train, \
                                                 self.stds_of_vars_bmsm_backtest, MSM_indicator=True, OLS_bool=False)
                self.name_bmsm = 'BMSM'
                ...
                self.cash_bmsm_bt, self.trading_portfolio_value_bmsm_bt, self.total_portfolio_value_bmsm_bt, \
                        ...
                        self.vega_bmsm_bt, self.gamma_bmsm_bt = \
                ...
                # do with OLS to compare without having to re-run estimations
                self.long_optimal_bmsm_ols, self.short_optimal_bmsm_ols, self.sig_mult_bmsm_ols = \
                        self.get_hyperparameters(self.forecasts_bmsm_backtest, self.signal_bmsm_backtest, self.forecasts_train_ols_bmsm_backtest, \
                                                 self.true_var_series_train_ols_bmsm_backtest, self.stds_of_vars_bmsm_backtest_train, \
                                                 self.stds_of_vars_bmsm_backtest, MSM_indicator=True, OLS_bool=True)

                X, Xt, b, y_log, m_log, m_logt = get_regression_features_MSM(self.forecasts_train_ols_bmsm_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_bmsm_backtest)], \
                                                                            self.stds_of_vars_bmsm_backtest_train, self.true_var_series_train_ols_bmsm_backtest, self.Pi_train_backtest,\
                                                                             self.forecasts_bmsm_backtest, self.vol_smile_backtest, self.stds_of_vars_bmsm_backtest, self.Pi_test_backtest, self.bit_states_backtest, self.m0_backtest)
                
                self.forecasts_bmsm_backtest_ols, self.smear_bmsm, self.model_bmsm, self.scalar_bmsm = regression_forecasts_train(\
                    X, b, y_log, m_log, m_logt, Xt)

                self.name_bmsm_ols = "BMSM with OLS"
                
                self.cash_bmsm_bt_ols, self.trading_portfolio_value_bmsm_bt_ols, self.total_portfolio_value_bmsm_bt_ols, \
                        self.trading_summary_bmsm_bt_ols, self.option_delta_positions_bmsm_bt_ols, self.delta_hedge_bmsm_bt_ols, \
                        self.delta_hedge_cashflows_bmsm_bt_ols, self.hedge_carry_bmsm_bt_ols, self.daily_option_pnl_real_bmsm_bt_ols, \
                        self.daily_option_pnl_mtm_bmsm_bt_ols,self.daily_hedge_pnl_mtm_bmsm_bt_ols,self.sharpe_bmsm_bt_ols, self.max_dd_bmsm_bt_ols, \
                        self.calmar_bmsm_bt_ols, self.cagr_bmsm_bt_ols, _, self.precision_all_bmsm_bt_ols, self.precision_active_bmsm_bt_ols, \
                        self.win_loss_ratio_all_bmsm_bt_ols, self.win_loss_ratio_active_bmsm_bt_ols, self.avg_win_all_bmsm_bt_ols, self.avg_win_active_bmsm_bt_ols, \
                        self.avg_loss_all_bmsm_bt_ols, self.avg_loss_active_bmsm_bt_ols, self.non_active_share_bmsm_bt_ols, self.total_trades_bmsm_bt_ols, \
                        self.vega_bmsm_bt_ols, self.gamma_bmsm_bt_ols = \
                                self.run_trading_strategy(self.forecasts_bmsm_backtest_ols, self.signal_bmsm_backtest, \
                                                        self.long_optimal_bmsm_ols, self.short_optimal_bmsm_ols, self.sig_mult_bmsm_ols, \
                                                        "Backtest", self.name_bmsm_ols, self.plots, verbose)
                ...
                self.cash_bmsm_oos, self.trading_portfolio_value_bmsm_oos, self.total_portfolio_value_bmsm_oos, \
                        ...
                        self.vega_bmsm_oos, self.gamma_bmsm_oos = \
                                self.run_trading_strategy(self.forecasts_bmsm_oos, self.signal_bmsm_oos, \
                                                        self.long_optimal_bmsm, self.short_optimal_bmsm, self.sig_mult_bmsm, \
                                                        "Out of Sample", self.name_bmsm, self.plots, verbose)
                
                X_oos, Xt_oos, b_oos, y_log_oos , m_log_oos, m_logt_oos = get_regression_features_MSM(self.forecasts_train_ols_bmsm_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_bmsm_oos)], \
                                                                            self.stds_of_vars_bmsm_oos_train, self.true_var_series_train_ols_bmsm_oos, self.Pi_train_oos,\
                                                                             self.forecasts_bmsm_oos, self.vol_smile_oos, self.stds_of_vars_bmsm_oos, self.Pi_test_oos, self.bit_states_oos, self.m0_oos)

                self.forecasts_bmsm_oos_ols,  self.smear_bmsm_oos, self.model_bmsm_oos, self.scalar_bmsm_oos = regression_forecasts_train(\
                    X_oos, b_oos, y_log_oos, m_log_oos, m_logt_oos, Xt_oos)

                self.cash_bmsm_oos_ols, self.trading_portfolio_value_bmsm_oos_ols, self.total_portfolio_value_bmsm_oos_ols, \
                        self.trading_summary_bmsm_oos_ols, self.option_delta_positions_bmsm_oos_ols, self.delta_hedge_bmsm_oos_ols, \
                        self.delta_hedge_cashflows_bmsm_oos_ols, self.hedge_carry_bmsm_oos_ols, self.daily_option_pnl_real_bmsm_oos_ols, \
                        self.daily_option_pnl_mtm_bmsm_oos_ols,self.daily_hedge_pnl_mtm_bmsm_oos_ols,self.sharpe_bmsm_oos_ols, self.max_dd_bmsm_oos_ols, \
                        self.calmar_bmsm_oos_ols, self.cagr_bmsm_oos_ols, _, self.precision_all_bmsm_oos_ols, self.precision_active_bmsm_oos_ols, \
                        self.win_loss_ratio_all_bmsm_oos_ols, self.win_loss_ratio_active_bmsm_oos_ols, self.avg_win_all_bmsm_oos_ols, self.avg_win_active_bmsm_oos_ols, \
                        self.avg_loss_all_bmsm_oos_ols, self.avg_loss_active_bmsm_oos_ols, self.non_active_share_bmsm_oos_ols, self.total_trades_bmsm_oos_ols, \
                        self.vega_bmsm_oos_ols, self.gamma_bmsm_oos_ols = \
                                self.run_trading_strategy(self.forecasts_bmsm_oos_ols, self.signal_bmsm_oos, \
                                                        self.long_optimal_bmsm_ols, self.short_optimal_bmsm_ols, self.sig_mult_bmsm_ols, \
                                                        "Out of Sample", self.name_bmsm_ols, self.plots, verbose)
        ...
        def get_GARCH_data(self):
                ...
                if self.garch_1_2_indicator:
                        ...
                        norm_mse, norm_mae, R2, realised_var_H_forecasts, realised_var_H_true, stds_of_vars, forecasts_train, true_vars_train, stds_of_vars_train, naive_forecast = garch_forecaster.forecast_garch()
                        self.log_L_garch, self.se_garch, self.alpha_p_garch, self.beta_p_garch, self.alpha_hat_garch, self.beta_hat_garch = garch_forecaster.access_extra_values()
                        ...
                        # collect decimal data for MSE/MAE analysis
                        self.forecasts_mse_mae_garch = realised_var_H_forecasts[1:] / (100**2)

                        # get decimal forecasts that line up with implied vols (1: so signal is based on our forecast for today vs implied vol yday)
                        realised_var_forecasts = realised_var_H_forecasts[1:] / 100
                        self.forecasts_garch_backtest = realised_var_forecasts[:int(self.N_test * self.train_size)]

                        stds_var = stds_of_vars[1:]
                        stds_var_backtest = stds_var[:int(self.N_test * self.train_size)]
                        signal_backtest = 1 / stds_var_backtest  
                        self.signal_garch_backtest = (signal_backtest - signal_backtest.min()) / (signal_backtest.max() - signal_backtest.min())

                        self.forecasts_train_ols_garch_backtest = forecasts_train / (100**2)
                        self.true_var_series_train_ols_garch_backtest = true_vars_train / (100**2)

                        self.stds_of_vars_garch_backtest_train = stds_of_vars_train
                        self.stds_of_vars_garch_backtest = stds_var_backtest
                        self.stds_vars_mse_mae_garch = stds_var
                        self.naive_forecast_garch_backtest = naive_forecast
                        ...
                        res = garch_forecaster.fit_garch_model_library()
                        norm_mse, norm_mae, R2, realised_var_H_forecasts, realised_var_H_true, stds_of_vars, forecasts_train, realised_var_H_true_train, stds_of_vars_train, naive_forecast = garch_forecaster.forecast_garch()
                        ...
                        # get decimal forecasts that line up with implied vols (1: so signal is based on our forecast for today vs implied vol yday)
                        realised_var_forecasts = realised_var_H_forecasts[1:] / (100 **2)
                        self.forecasts_garch_oos = realised_var_forecasts

                        stds_var = stds_of_vars[1:]
                        stds_var_oos = stds_var
                        signal_oos = 1 / stds_var_oos  
                        ...
                        self.forecasts_train_ols_garch_oos = forecasts_train / (100 ** 2)
                        self.true_var_series_train_ols_garch_oos = realised_var_H_true_train / (100 ** 2)

                        self.stds_of_vars_garch_oos_train = stds_of_vars_train
                        self.stds_of_vars_garch_oos = stds_of_vars[1:]
                        self.naive_forecast_garch_oos = naive_forecast
                        ...
                        norm_mse, norm_mae, R2, realised_var_H_forecasts, realised_var_H_true, stds_of_vars, forecasts_train, true_train, stds_of_vars_train, naive_forecast = garch_forecaster.forecast_garch()
                        self.log_L_garch, self.se_garch, self.alpha_p_garch, self.beta_p_garch, self.alpha_hat_garch, self.beta_hat_garch = garch_forecaster.access_extra_values()
                        ...
                        self.forecasts_train_ols_garch_backtest = forecasts_train / (100**2)
                        self.true_var_series_train_ols_garch_backtest = true_train / (100**2)
        ...
        def get_GARCH_hyperparams_and_run_strategy(self, verbose):
                ...
                self.long_optimal_garch, self.short_optimal_garch, self.sig_mult_garch = \
                        self.get_hyperparameters(self.forecasts_garch_backtest, self.signal_garch_backtest, self.forecasts_train_ols_garch_backtest, \
                                                 self.true_var_series_train_ols_garch_backtest, self.stds_of_vars_garch_backtest_train, \
                                                 self.stds_of_vars_garch_backtest, MSM_indicator=False, OLS_bool=False)
                ...
                self.cash_garch_bt, self.trading_portfolio_value_garch_bt, self.total_portfolio_value_garch_bt, \
                        ...
                        self.vega_garch_bt, self.gamma_garch_bt = \
                ...
                # do with OLS to compare without having to re-run estimations
                self.long_optimal_garch_ols, self.short_optimal_garch_ols, self.sig_mult_garch_ols = \
                        self.get_hyperparameters(self.forecasts_garch_backtest, self.signal_garch_backtest, self.forecasts_train_ols_garch_backtest, \
                                                 self.true_var_series_train_ols_garch_backtest, self.stds_of_vars_garch_backtest_train, \
                                                 self.stds_of_vars_garch_backtest, MSM_indicator=False, OLS_bool=True)

                X, Xt, b, y_log, m_log, m_logt = get_regression_features_OTHER(self.forecasts_train_ols_garch_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_garch_backtest)], \
                                                                            self.stds_of_vars_garch_backtest_train, self.true_var_series_train_ols_garch_backtest, 
                                                                            self.forecasts_garch_backtest, self.vol_smile_backtest, self.stds_of_vars_garch_backtest)

                self.forecasts_garch_backtest_ols, self.smear_garch, self.model_garch, self.scalar_garch = regression_forecasts_train(\
                    X, b, y_log, m_log, m_logt, Xt)

                self.name_garch_ols = "GARCH with OLS"

                self.cash_garch_bt_ols, self.trading_portfolio_value_garch_bt_ols, self.total_portfolio_value_garch_bt_ols, \
                        self.trading_summary_garch_bt_ols, self.option_delta_positions_garch_bt_ols, self.delta_hedge_garch_bt_ols, \
                        self.delta_hedge_cashflows_garch_bt_ols, self.hedge_carry_garch_bt_ols, self.daily_option_pnl_real_garch_bt_ols, \
                        self.daily_option_pnl_mtm_garch_bt_ols,self.daily_hedge_pnl_mtm_garch_bt_ols,self.sharpe_garch_bt_ols, self.max_dd_garch_bt_ols, \
                        self.calmar_garch_bt_ols, self.cagr_garch_bt_ols, _, self.precision_all_garch_bt_ols, self.precision_active_garch_bt_ols, \
                        self.win_loss_ratio_all_garch_bt_ols, self.win_loss_ratio_active_garch_bt_ols, self.avg_win_all_garch_bt_ols, self.avg_win_active_garch_bt_ols, \
                        self.avg_loss_all_garch_bt_ols, self.avg_loss_active_garch_bt_ols, self.non_active_share_garch_bt_ols, self.total_trades_garch_bt_ols,\
                        self.vega_garch_bt_ols, self.gamma_garch_bt_ols = \
                                self.run_trading_strategy(self.forecasts_garch_backtest_ols, self.signal_garch_backtest, \
                                                        self.long_optimal_garch_ols, self.short_optimal_garch_ols, self.sig_mult_garch_ols, \
                                                        "Backtest", self.name_garch_ols, self.plots, verbose)
                ...
                self.cash_garch_oos, self.trading_portfolio_value_garch_oos, self.total_portfolio_value_garch_oos, \
                        ...
                        self.vega_garch_oos, self.gamma_garch_oos = \
                                self.run_trading_strategy(self.forecasts_garch_oos, self.signal_garch_oos, \
                                                        self.long_optimal_garch, self.short_optimal_garch, self.sig_mult_garch, \
                                                        "Out of Sample", self.name_garch, self.plots, verbose)
     
                X_oos, Xt_oos, b_oos, y_log_oos, m_log_oos, m_logt_oos = get_regression_features_OTHER(self.forecasts_train_ols_garch_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_garch_oos)], \
                                                                            self.stds_of_vars_garch_oos_train, self.true_var_series_train_ols_garch_oos,\
                                                                             self.forecasts_garch_oos, self.vol_smile_oos, self.stds_of_vars_garch_oos)

                self.forecasts_garch_oos_ols,  self.smear_garch_oos, self.model_garch_oos, self.scalar_garch_oos = regression_forecasts_train(\
                    X_oos, b_oos, y_log_oos, m_log_oos, m_logt_oos, Xt_oos)
                
                self.cash_garch_oos_ols, self.trading_portfolio_value_garch_oos_ols, self.total_portfolio_value_garch_oos_ols, \
                        self.trading_summary_garch_oos_ols, self.option_delta_positions_garch_oos_ols, self.delta_hedge_garch_oos_ols, \
                        self.delta_hedge_cashflows_garch_oos_ols, self.hedge_carry_garch_oos_ols, self.daily_option_pnl_real_garch_oos_ols, \
                        self.daily_option_pnl_mtm_garch_oos_ols, self.daily_hedge_pnl_mtm_garch_oos_ols, self.sharpe_garch_oos_ols, self.max_dd_garch_oos_ols, \
                        self.calmar_garch_oos_ols, self.cagr_garch_oos_ols, _, self.precision_all_garch_oos_ols, self.precision_active_garch_oos_ols, \
                        self.win_loss_ratio_all_garch_oos_ols, self.win_loss_ratio_active_garch_oos_ols, self.avg_win_all_garch_oos_ols, self.avg_win_active_garch_oos_ols, \
                        self.avg_loss_all_garch_oos_ols, self.avg_loss_active_garch_oos_ols, self.non_active_share_garch_oos_ols, self.total_trades_garch_oos_ols, \
                        self.vega_garch_oos, self.gamma_garch_oos = \
                                self.run_trading_strategy(self.forecasts_garch_oos_ols, self.signal_garch_oos, \
                                                        self.long_optimal_garch_ols, self.short_optimal_garch_ols, self.sig_mult_garch_ols, \
                                                        "Out of Sample", self.name_garch_ols, self.plots, verbose)
        ...
        def get_FIGARCH_data(self):
                ...
                figarch_forecaster_bt = FIGARCH_Forecaster_1_d_0_MLE_delta_stds(
                        ...
                        scale=100,
                        percent_space=True, 
                        beta_zero=self.FIGARCH_ind)

                omega, d, beta, sigma2_prev = figarch_forecaster_bt.estimate_figarch()
                norm_mse, norm_mae, R2, realised_var_H_forecasts, realised_var_H_true, stds_of_vars, forecasts_train, true_vars_train, stds_of_vars_train, naive_forecast = figarch_forecaster_bt.forecast_figarch()
                self.log_L_figarch, self.se_figarch, self.alpha_p_figarch, self.beta_p_figarch, self.alpha_hat_figarch, self.beta_hat_figarch = figarch_forecaster_bt.access_extra_values()
                ...
                # collect decimal data for MSE/MAE analysis
                self.forecasts_mse_mae_figarch = realised_var_H_forecasts[1:] / (100**2)
       
                # get decimal forecasts that line up with implied vols (1: so signal is based on our forecast for today vs implied vol yday)
                realised_var_forecasts = realised_var_H_forecasts[1:] / (100**2)
                self.forecasts_figarch_backtest = realised_var_forecasts[:int(self.N_test * self.train_size)]
                ...
                self.true_var_series_train_ols_figarch_backtest = true_vars_train / 100

                self.stds_of_vars_figarch_backtest_train = stds_of_vars_train
                self.stds_of_vars_figarch_backtest = stds_var_backtest
                self.stds_vars_mse_mae_figarch = stds_var
                self.naive_forecast_figarch_backtest = naive_forecast
                ...
                norm_mse, norm_mae, R2, realised_var_H_forecasts, realised_var_H_true, stds_of_vars, forecasts_train, realised_var_H_true_train, stds_of_vars_train, naive_forecast = figarch_forecaster_oos.forecast_figarch()
                ...
                # get decimal forecasts that line up with implied vols - again one forward so no forward look when comparing to implied vols
                self.forecasts_figarch_oos = realised_var_H_forecasts[1:] / (100**2)
                signal_oos = 1 / stds_of_vars[1:]  
                ...
                self.forecasts_train_ols_figarch_oos = forecasts_train / (100**2)
                self.true_var_series_train_ols_figarch_oos = realised_var_H_true_train / (100**2)

                self.stds_of_vars_figarch_oos_train = stds_of_vars_train
                self.stds_of_vars_figarch_oos = stds_of_vars[1:]
                self.naive_forecast_figarch_oos = naive_forecast
        ...
        def get_FIGARCH_hyperparams_and_run_strategy(self, verbose):
            ...
            self.long_optimal_figarch, self.short_optimal_figarch, self.sig_mult_figarch = \
                        self.get_hyperparameters(self.forecasts_figarch_backtest, self.signal_figarch_backtest, self.forecasts_train_ols_figarch_backtest, \
                                                 self.true_var_series_train_ols_figarch_backtest, self.stds_of_vars_figarch_backtest_train, \
                                                 self.stds_of_vars_figarch_backtest, MSM_indicator=False, OLS_bool=False)
            ...
            self.cash_figarch_bt, self.trading_portfolio_value_figarch_bt, self.total_portfolio_value_figarch_bt, \
                        ...
                        self.vega_figarch_bt, self.gamma_figarch_bt = \
            ...
            # do with OLS to compare without having to re-run estimations
            self.long_optimal_figarch_ols, self.short_optimal_figarch_ols, self.sig_mult_figarch_ols = \
                        self.get_hyperparameters(self.forecasts_figarch_backtest, self.signal_figarch_backtest, self.forecasts_train_ols_figarch_backtest, \
                                                 self.true_var_series_train_ols_figarch_backtest, self.stds_of_vars_figarch_backtest_train, \
                                                 self.stds_of_vars_figarch_backtest, MSM_indicator=False, OLS_bool=True)

            X, Xt, b, y_log, m_log, m_logt = get_regression_features_OTHER(self.forecasts_train_ols_figarch_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_figarch_backtest)], \
                                                                            self.stds_of_vars_figarch_backtest_train, self.true_var_series_train_ols_figarch_backtest, 
                                                                            self.forecasts_figarch_backtest, self.vol_smile_backtest, self.stds_of_vars_figarch_backtest)

            self.forecasts_figarch_backtest_ols, self.smear_figarch, self.model_figarch, self.scalar_figarch = regression_forecasts_train(\
                    X, b, y_log, m_log, m_logt, Xt)

            self.name_figarch_ols = "FIGARCH with OLS"

            self.cash_figarch_bt_ols, self.trading_portfolio_value_figarch_bt_ols, self.total_portfolio_value_figarch_bt_ols, \
                        self.trading_summary_figarch_bt_ols, self.option_delta_positions_figarch_bt_ols, self.delta_hedge_figarch_bt_ols, \
                        self.delta_hedge_cashflows_figarch_bt_ols, self.hedge_carry_figarch_bt_ols, self.daily_option_pnl_real_figarch_bt_ols, \
                        self.daily_option_pnl_mtm_figarch_bt_ols,self.daily_hedge_pnl_mtm_figarch_bt_ols,self.sharpe_figarch_bt_ols, self.max_dd_figarch_bt_ols, \
                        self.calmar_figarch_bt_ols, self.cagr_figarch_bt_ols, _, self.precision_all_figarch_bt_ols, self.precision_active_figarch_bt_ols, \
                        self.win_loss_ratio_all_figarch_bt_ols, self.win_loss_ratio_active_figarch_bt_ols, self.avg_win_all_figarch_bt_ols, self.avg_win_active_figarch_bt_ols, \
                        self.avg_loss_all_figarch_bt_ols, self.avg_loss_active_figarch_bt_ols, self.non_active_share_figarch_bt_ols, self.total_trades_figarch_bt_ols, \
                        self.vega_figarch_bt_ols, self.gamma_figarch_bt_ols = \
                                self.run_trading_strategy(self.forecasts_figarch_backtest_ols, self.signal_figarch_backtest, \
                                                        self.long_optimal_figarch_ols, self.short_optimal_figarch_ols, self.sig_mult_figarch_ols, \
                                                        "Backtest", self.name_figarch_ols, self.plots, verbose)
            ...
            self.cash_figarch_oos, self.trading_portfolio_value_figarch_oos, self.total_portfolio_value_figarch_oos, \
                        ...
                        self.vega_figarch_oos, self.gamma_figarch_oos = \
            ...
            X_oos, Xt_oos, b_oos, y_log_oos, m_log_oos, m_logt_oos = get_regression_features_OTHER(self.forecasts_train_ols_figarch_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_figarch_oos)], \
                                                                            self.stds_of_vars_figarch_oos_train, self.true_var_series_train_ols_figarch_oos,\
                                                                             self.forecasts_figarch_oos, self.vol_smile_oos, self.stds_of_vars_figarch_oos)

            self.forecasts_figarch_oos_ols,  self.smear_figarch_oos, self.model_figarch_oos, self.scalar_figarch_oos = regression_forecasts_train(\
                    X_oos, b_oos, y_log_oos, m_log_oos, m_logt_oos, Xt_oos)

            self.cash_figarch_oos_ols, self.trading_portfolio_value_figarch_oos_ols, self.total_portfolio_value_figarch_oos_ols, \
                        self.trading_summary_figarch_oos_ols, self.option_delta_positions_figarch_oos_ols, self.delta_hedge_figarch_oos_ols, \
                        self.delta_hedge_cashflows_figarch_oos_ols, self.hedge_carry_figarch_oos_ols, self.daily_option_pnl_real_figarch_oos_ols, \
                        self.daily_option_pnl_mtm_figarch_oos_ols, self.daily_hedge_pnl_mtm_figarch_oos_ols, self.sharpe_figarch_oos_ols, self.max_dd_figarch_oos_ols, \
                        self.calmar_figarch_oos_ols, self.cagr_figarch_oos_ols, _, self.precision_all_figarch_oos_ols, self.precision_active_figarch_oos_ols, \
                        self.win_loss_ratio_all_figarch_oos_ols, self.win_loss_ratio_active_figarch_oos_ols, self.avg_win_all_figarch_oos_ols, self.avg_win_active_figarch_oos_ols, \
                        self.avg_loss_all_figarch_oos_ols, self.avg_loss_active_figarch_oos_ols, self.non_active_share_figarch_oos_ols, self.total_trades_figarch_oos_ols, \
                        self.vega_figarch_oos_ols, self.gamma_figarch_oos_ols = \
                                self.run_trading_strategy(self.forecasts_figarch_oos_ols, self.signal_figarch_oos, \
                                                        self.long_optimal_figarch, self.short_optimal_figarch, self.sig_mult_figarch, \
                                                        "Out of Sample", self.name_figarch_ols, self.plots, verbose)
        ...
        def compare_strategies(self, verbose):
                ...
                COLORS = {
                        "actual": "#4812AB",
                        "iv": "#000000",
                        "BMSM_raw":  "#0072B2",   "BMSM_ols": "#9FC5E8",
                        "GARCH_raw":"#E66F00",   "GARCH_ols":"#FFD27F",
                        "FIG_raw":  "#E20DDE",   "FIG_ols":  "#E2A4F0",
                        "BUY": "#33BE05",
                        "SELL":"#FF0303",
                    }
                ...
                plt.plot(self.total_portfolio_value_bmsm_oos, label=f'{self.name_bmsm} Total Portfolio Value', color=COLORS['BMSM_raw'])
                plt.plot(self.total_portfolio_value_bmsm_oos_ols, label=f'{self.name_bmsm_ols} Total Portfolio Value', color=COLORS['BMSM_ols'])
                plt.plot(self.total_portfolio_value_garch_oos, label=f'{self.name_garch} Total Portfolio Value', color=COLORS['GARCH_raw'], linestyle = ":")
                plt.plot(self.total_portfolio_value_garch_oos_ols, label=f'{self.name_garch_ols} Total Portfolio Value', color=COLORS['GARCH_ols'], linestyle = ":")
                plt.plot(self.total_portfolio_value_figarch_oos, label=f'{self.name_figarch} Total Portfolio Value', color=COLORS['FIG_raw'], linestyle='--')
                plt.plot(self.total_portfolio_value_figarch_oos_ols, label=f'{self.name_figarch_ols} Total Portfolio Value', color=COLORS['FIG_ols'], linestyle='--')
                plt.xlabel("Day", fontsize=16)
                plt.ylabel("Total Portfolio Value", fontsize=16)
                ...
                plt.figure(figsize=(8, 6))
                plt.plot(self.total_portfolio_value_bmsm_bt, label=f'{self.name_bmsm} Total Portfolio Value', color=COLORS['BMSM_raw'])
                plt.plot(self.total_portfolio_value_bmsm_bt_ols, label=f'{self.name_bmsm_ols} Total Portfolio Value',  color=COLORS['BMSM_ols'])
                plt.plot(self.total_portfolio_value_garch_bt, label=f'{self.name_garch} Total Portfolio Value', color=COLORS['GARCH_raw'], linestyle = ":")
                plt.plot(self.total_portfolio_value_garch_bt_ols, label=f'{self.name_garch_ols} Total Portfolio Value', color=COLORS['GARCH_ols'], linestyle = ":")
                plt.plot(self.total_portfolio_value_figarch_bt, label=f'{self.name_figarch} Total Portfolio Value',  color=COLORS['FIG_raw'], linestyle='--')
                plt.plot(self.total_portfolio_value_figarch_bt_ols, label=f'{self.name_figarch_ols} Total Portfolio Value', color=COLORS['FIG_ols'], linestyle='--')
                plt.xlabel("Day", fontsize=16)
                plt.ylabel("Total Portfolio Value", fontsize=16)
                ...
                '''pairs = [
                    (self.name_bmsm,       self.forecasts_bmsm_backtest,       self.forecasts_bmsm_oos,
                    self.name_bmsm_ols,   self.forecasts_bmsm_backtest_ols,   self.forecasts_bmsm_oos_ols,
                    ("#075f45", "#50cc12"), self.trading_summary_bmsm_bt_ols, self.trading_summary_bmsm_oos_ols),  # green pair

                    (self.name_garch,      self.forecasts_garch_backtest,      self.forecasts_garch_oos,
                    self.name_garch_ols,  self.forecasts_garch_backtest_ols,  self.forecasts_garch_oos_ols,
                    ("#0B416E", "#15afc3"), self.trading_summary_garch_bt_ols, self.trading_summary_garch_oos_ols),  # blue pair

                    (self.name_figarch,    self.forecasts_figarch_backtest,    self.forecasts_figarch_oos,
                    self.name_figarch_ols,self.forecasts_figarch_backtest_ols,self.forecasts_figarch_oos_ols,
                    ("#e41a1c", "#e94795"), self.trading_summary_figarch_bt_ols, self.trading_summary_figarch_oos_ols),  # red pair
                ]

                fig, axs = plt.subplots(3, 1, figsize=(14, 10), sharex=True)
                backtest_idx = len(self.forecasts_bmsm_backtest)

                for ax, (name_raw, fc_bt_raw, fc_oos_raw,
                        name_ols, fc_bt_ols, fc_oos_ols, (c_raw, c_ols), summary_bt_ols, summary_oos_ols) in zip(axs, pairs):

                    # actual + iv
                    ax.plot(self.true_vol_series * 100, lw=1.6, ls=':', label='Actual Realised', color="#4812AB")
                    ax.plot(self.atm_mid_implied_vol_test * 100, lw=1.2, ls='--', label='ATM Mid Implied', color='black', alpha=0.8)

                    # forecasts (raw + OLS) on same axis
                    ax.plot(np.hstack((fc_bt_raw, fc_oos_raw)) * 100, lw=1, ls='-',  label=f'{name_raw} Forecast', color=c_raw, alpha=0.9)
                    ax.plot(np.hstack((fc_bt_ols, fc_oos_ols)) * 100, lw=1, ls='-',  label=f'{name_ols} Forecast', color=c_ols, alpha=0.9)

                    # get buy/sell signals
                    pos_bt = np.array(summary_bt_ols['Option Positions'])
                    pos_oos = np.array(summary_oos_ols['Option Positions'])
                    pos_prev_bt, pos_curr_bt = pos_bt[:-1], pos_bt[1:]
                    pos_prev_oos, pos_curr_oos = pos_oos[:-1], pos_oos[1:]
                    t_idx_bt = np.arange(1, len(pos_bt))
                    t_idx_oos = np.arange(1 + len(pos_bt), len(pos_bt) + len(pos_oos))

                    buy_signals_bt = t_idx_bt[(pos_prev_bt == 0) & (pos_curr_bt == 1)]
                    sell_signals_bt = t_idx_bt[(pos_prev_bt == 0) & (pos_curr_bt == -1)]

                    buy_signals_oos = t_idx_oos[(pos_prev_oos == 0) & (pos_curr_oos == 1)]
                    sell_signals_oos = t_idx_oos[(pos_prev_oos == 0) & (pos_curr_oos == -1)]

                    ax.scatter(buy_signals_bt, fc_bt_ols[buy_signals_bt]*100,
                            marker='^', color='green', s=10, label='Buy Signal')
                    ax.scatter(sell_signals_bt, fc_bt_ols[sell_signals_bt]*100,
                            marker='v', color='red', s=10, label='Sell Signal')
                    ax.scatter(buy_signals_oos, fc_oos_ols[buy_signals_oos]*100,
                            marker='^', color='green', s=10)
                    ax.scatter(sell_signals_oos, fc_oos_ols[sell_signals_oos]*100,
                            marker='v', color='red', s=10)
                    ...
                    ax.set_title(f'{name_raw} vs. {name_ols}: Forecasts vs. Realised Volatility')

                axs[-1].set_xlabel('Date', fontsize=14)
                fig.suptitle(f'{self.H}-Day Annualised Volatility: Raw vs OLS for {self.ticker}', fontsize=18, y=1.02)
                fig.tight_layout()'''

                pairs = [
                    (self.name_bmsm, self.forecasts_bmsm_backtest, self.forecasts_bmsm_oos,
                    self.name_bmsm_ols, self.forecasts_bmsm_backtest_ols, self.forecasts_bmsm_oos_ols,
                    (COLORS["BMSM_raw"], COLORS["BMSM_ols"]), self.trading_summary_bmsm_bt_ols, self.trading_summary_bmsm_oos_ols,\
                        self.total_portfolio_value_bmsm_bt_ols, self.total_portfolio_value_bmsm_oos_ols)]
                ''',

                    (self.name_garch, self.forecasts_garch_backtest, self.forecasts_garch_oos,
                    self.name_garch_ols, self.forecasts_garch_backtest_ols, self.forecasts_garch_oos_ols,
                    (COLORS["GARCH_raw"], COLORS["GARCH_ols"]), self.trading_summary_garch_bt_ols, self.trading_summary_garch_oos_ols, \
                        self.total_portfolio_value_garch_bt_ols, self.total_portfolio_value_garch_oos_ols),

                    (self.name_figarch, self.forecasts_figarch_backtest, self.forecasts_figarch_oos,
                    self.name_figarch_ols, self.forecasts_figarch_backtest_ols, self.forecasts_figarch_oos_ols,
                    (COLORS["FIG_raw"], COLORS["FIG_ols"]), self.trading_summary_figarch_bt_ols, self.trading_summary_figarch_oos_ols, \
                        self.total_portfolio_value_figarch_bt_ols, self.total_portfolio_value_figarch_oos_ols),
                ]'''

                fig = plt.figure(figsize=(14, 10))
                gs = GridSpec(nrows=2*len(pairs), ncols=1, height_ratios=[4,1]*len(pairs), hspace=0.1) #0.3
                backtest_idx = len(self.forecasts_bmsm_backtest)
                first_top_ax = None

                for i, ((name_raw, fc_bt_raw, fc_oos_raw,
                            name_ols, fc_bt_ols, fc_oos_ols, (c_raw, c_ols), summary_bt_ols, summary_oos_ols, V_bt, V_oos)) \
                            in enumerate(pairs):
                    ...
                    if first_top_ax is None:
                        ...
                        ax = fig.add_subplot(gs[i*2], sharex=first_top_ax)
                        ax.tick_params(axis='x', which='both', labelbottom=False)

                    ax_pnl = fig.add_subplot(gs[i*2 + 1], sharex=ax)

                    ax.plot(np.sqrt(self.true_var_series)*100, lw=1.5, ls='--', label='Actual Realised',
                            color=COLORS["actual"])
                    ax.plot(self.atm_mid_implied_vol_test*100, lw=1.3, ls='-', label='ATM Mid Implied',
                            color=COLORS["iv"], alpha=0.8)

                    #ax.plot(np.sqrt(np.hstack((fc_bt_raw, fc_oos_raw)))*100, lw=1.2, ls='-',
                            #label=f'{name_raw} Forecast', color=c_raw, alpha=0.95)
                    ax.plot(np.sqrt(np.hstack((fc_bt_ols, fc_oos_ols)))*100, lw=1.2, ls='-',
                            label=f'{name_ols} Forecast', color=c_ols, alpha=0.95)
                    ...
                    buy_bt  = t_idx_bt[(pos_bt[:-1] == 0) & (pos_bt[1:] == 1)]
                    sell_bt = t_idx_bt[(pos_bt[:-1] == 0) & (pos_bt[1:] == -1)]
                    buy_oos  = t_idx_oos[(pos_oos[:-1] == 0) & (pos_oos[1:] == 1)]
                    sell_oos = t_idx_oos[(pos_oos[:-1] == 0) & (pos_oos[1:] == -1)]

                    ax.scatter(buy_bt,  np.sqrt(fc_bt_ols[buy_bt - 1])*100,  marker='^', s=48,
                            facecolors='none', edgecolors=COLORS["BUY"], linewidths=1.8,
                            zorder=5, label='Long Signal' if i == 0 else None)
                    ax.scatter(sell_bt, np.sqrt(fc_bt_ols[sell_bt - 1])*100, marker='v', s=48,
                            facecolors='none', edgecolors=COLORS["SELL"], linewidths=1.8,
                            zorder=5, label='Short Signal' if i == 0 else None)
                    ax.scatter(buy_oos,  np.sqrt(fc_oos_ols[buy_oos - len(pos_bt) - 1])*100,  marker='^', s=48,
                            facecolors='none', edgecolors=COLORS["BUY"], linewidths=1.8, zorder=5)
                    ax.scatter(sell_oos, np.sqrt(fc_oos_ols[sell_oos - len(pos_bt) - 1])*100, marker='v', s=48,
                            facecolors='none', edgecolors=COLORS["SELL"], linewidths=1.8, zorder=5)

                    ax.axvline(x=backtest_idx, color='gray', linestyle='--', lw=1, label='Backtest Split' if i == 0 else None)

                    ax.set_ylabel('Volatility (%)', fontsize=16)
                    ax.grid(alpha=0.35)
                    ax.set_title(f'{name_ols}: Forecasts vs Realised Volatility and Daily MtM Portfolio Returns', fontsize = 18)

                    V_returns_bt = np.log(V_bt[1:] / V_bt[:-1])
                    V_returns_oos = np.log(V_oos[1:] / V_oos[:-1])
                    pnl = np.concatenate((V_returns_bt, V_returns_oos)) * 100
                    x_pos = np.arange(len(pnl))
 
                    gain   = np.where(pnl > 0, pnl, 0)
                    loss   = np.where(pnl < 0, pnl, 0)

                    ax_pnl.bar(x_pos, gain,  width=3.0, color='limegreen', alpha=.75)
                    ax_pnl.bar(x_pos, loss,  width=3.0, color='crimson' , alpha=.75)
                    ax_pnl.axhline(0, color='k', lw=0.8, alpha=0.6)
                    ax_pnl.grid(alpha=0.25)
                    ax_pnl.set_ylabel('Returns (%)', fontsize=16)

                    if i < len(pairs) - 1:
                        ax_pnl.tick_params(axis='x', labelbottom=False, labelsize = 15) 
                    ...
                    ax.legend(clean_handles, clean_labels, loc='upper left',
                            frameon=True, facecolor='white', edgecolor='black',
                            framealpha=0.8, fontsize=14)

                ax_pnl.set_xlabel('Date', fontsize=16)
                fig.suptitle(f'{self.H}-Day Annualised Volatility: OLS Correction for {self.ticker}',  fontweight = 'bold', fontsize=24, y=0.995)
                fig.tight_layout()
                ...
                summary_in_domestic = {
                        ...
                        f"{self.name_bmsm_ols} Backtest": {
                                "long_threshold": self.long_optimal_bmsm_ols,
                                "short_threshold": self.short_optimal_bmsm_ols,
                                "signal_multiplier": self.sig_mult_bmsm_ols,
                                "sharpe_ratio (USD)": self.sharpe_bmsm_bt_ols,
                                "max_drawdown (USD)": self.max_dd_bmsm_bt_ols,
                                "calmar_ratio (USD)": self.calmar_bmsm_bt_ols,
                                "cagr (USD)": self.cagr_bmsm_bt_ols,
                                "Precision (all)": self.precision_all_bmsm_bt_ols,
                                "Precision (active)": self.precision_active_bmsm_bt_ols,
                                "Win/loss ratio (all)": self.win_loss_ratio_all_bmsm_bt_ols,
                                "Win/loss ratio (active)": self.win_loss_ratio_active_bmsm_bt_ols,
                                "Avg win (all)": self.avg_win_all_bmsm_bt_ols,
                                "Avg win (active)": self.avg_win_active_bmsm_bt_ols,
                                "Avg loss (all)": self.avg_loss_all_bmsm_bt_ols,
                                "Avg loss (active)": self.avg_loss_active_bmsm_bt_ols,
                                "Non-active share": self.non_active_share_bmsm_bt_ols,
                                "Total trades": self.total_trades_bmsm_bt_ols
                        },

                        f"{self.name_bmsm_ols} Out of Sample": {
                                "long_threshold": self.long_optimal_bmsm_ols,
                                "short_threshold": self.short_optimal_bmsm_ols,
                                "signal_multiplier": self.sig_mult_bmsm_ols,
                                "sharpe_ratio (USD)": self.sharpe_bmsm_oos_ols,
                                "max_drawdown (USD)": self.max_dd_bmsm_oos_ols,
                                "calmar_ratio (USD)": self.calmar_bmsm_oos_ols,
                                "cagr (USD)": self.cagr_bmsm_oos_ols,
                                "Precision (all)": self.precision_all_bmsm_oos_ols,
                                "Precision (active)": self.precision_active_bmsm_oos_ols,
                                "Win/loss ratio (all)": self.win_loss_ratio_all_bmsm_oos_ols,
                                "Win/loss ratio (active)": self.win_loss_ratio_active_bmsm_oos_ols,
                                "Avg win (all)": self.avg_win_all_bmsm_oos_ols,
                                "Avg win (active)": self.avg_win_active_bmsm_oos_ols,
                                "Avg loss (all)": self.avg_loss_all_bmsm_oos_ols,
                                "Avg loss (active)": self.avg_loss_active_bmsm_oos_ols,
                                "Non-active share": self.non_active_share_bmsm_oos_ols,
                                "Total trades": self.total_trades_bmsm_oos_ols
                        },
                        ...
                        f"{self.name_garch_ols} Backtest": {
                                "long_threshold": self.long_optimal_garch_ols,
                                "short_threshold": self.short_optimal_garch_ols,
                                "signal_multiplier": self.sig_mult_garch_ols,
                                "sharpe_ratio (USD)": self.sharpe_garch_bt_ols,
                                "max_drawdown (USD)": self.max_dd_garch_bt_ols,
                                "calmar_ratio (USD)": self.calmar_garch_bt_ols,
                                "cagr (USD)": self.cagr_garch_bt_ols,
                                "Precision (all)": self.precision_all_garch_bt_ols,
                                "Precision (active)": self.precision_active_garch_bt_ols,
                                "Win/loss ratio (all)": self.win_loss_ratio_all_garch_bt_ols,
                                "Win/loss ratio (active)": self.win_loss_ratio_active_garch_bt_ols,
                                "Avg win (all)": self.avg_win_all_garch_bt_ols,
                                "Avg win (active)": self.avg_win_active_garch_bt_ols,
                                "Avg loss (all)": self.avg_loss_all_garch_bt_ols,
                                "Avg loss (active)": self.avg_loss_active_garch_bt_ols,
                                "Non-active share": self.non_active_share_garch_bt_ols,
                                "Total trades": self.total_trades_garch_bt_ols
                        },

                        f"{self.name_garch_ols} Out of Sample": {
                                "long_threshold": self.long_optimal_garch_ols,
                                "short_threshold": self.short_optimal_garch_ols,
                                "signal_multiplier": self.sig_mult_garch_ols,
                                "sharpe_ratio (USD)": self.sharpe_garch_oos_ols,
                                "max_drawdown (USD)": self.max_dd_garch_oos_ols,
                                "calmar_ratio (USD)": self.calmar_garch_oos_ols,
                                "cagr (USD)": self.cagr_garch_oos_ols,
                                "Precision (all)": self.precision_all_garch_oos_ols,
                                "Precision (active)": self.precision_active_garch_oos_ols,
                                "Win/loss ratio (all)": self.win_loss_ratio_all_garch_oos_ols,
                                "Win/loss ratio (active)": self.win_loss_ratio_active_garch_oos_ols,
                                "Avg win (all)": self.avg_win_all_garch_oos_ols,
                                "Avg win (active)": self.avg_win_active_garch_oos_ols,
                                "Avg loss (all)": self.avg_loss_all_garch_oos_ols,
                                "Avg loss (active)": self.avg_loss_active_garch_oos_ols,
                                "Non-active share": self.non_active_share_garch_oos_ols,
                                "Total trades": self.total_trades_garch_oos_ols
                        },
                        ...
                        f"{self.name_figarch_ols} Backtest": {
                                "long_threshold": self.long_optimal_figarch_ols,
                                "short_threshold": self.short_optimal_figarch_ols,
                                "signal_multiplier": self.sig_mult_figarch_ols,
                                "sharpe_ratio (USD)": self.sharpe_figarch_bt_ols,
                                "max_drawdown (USD)": self.max_dd_figarch_bt_ols,
                                "calmar_ratio (USD)": self.calmar_figarch_bt_ols,
                                "cagr (USD)": self.cagr_figarch_bt_ols,
                                "Precision (all)": self.precision_all_figarch_bt_ols,
                                "Precision (active)": self.precision_active_figarch_bt_ols,
                                "Win/loss ratio (all)": self.win_loss_ratio_all_figarch_bt_ols,
                                "Win/loss ratio (active)": self.win_loss_ratio_active_figarch_bt_ols,
                                "Avg win (all)": self.avg_win_all_figarch_bt_ols,
                                "Avg win (active)": self.avg_win_active_figarch_bt_ols,
                                "Avg loss (all)": self.avg_loss_all_figarch_bt_ols,
                                "Avg loss (active)": self.avg_loss_active_figarch_bt_ols,
                                "Non-active share": self.non_active_share_figarch_bt_ols,
                                "Total trades": self.total_trades_figarch_bt_ols
                        },

                        f"{self.name_figarch_ols} Out of Sample": {
                                "long_threshold": self.long_optimal_figarch_ols,
                                "short_threshold": self.short_optimal_figarch_ols,
                                "signal_multiplier": self.sig_mult_figarch_ols,
                                "sharpe_ratio (USD)": self.sharpe_figarch_oos_ols,
                                "max_drawdown (USD)": self.max_dd_figarch_oos_ols,
                                "calmar_ratio (USD)": self.calmar_figarch_oos_ols,
                                "cagr (USD)": self.cagr_figarch_oos_ols,
                                "Precision (all)": self.precision_all_figarch_oos_ols,
                                "Precision (active)": self.precision_active_figarch_oos_ols,
                                "Win/loss ratio (all)": self.win_loss_ratio_all_figarch_oos_ols,
                                "Win/loss ratio (active)": self.win_loss_ratio_active_figarch_oos_ols,
                                "Avg win (all)": self.avg_win_all_figarch_oos_ols,
                                "Avg win (active)": self.avg_win_active_figarch_oos_ols,
                                "Avg loss (all)": self.avg_loss_all_figarch_oos_ols,
                                "Avg loss (active)": self.avg_loss_active_figarch_oos_ols,
                                "Non-active share": self.non_active_share_figarch_oos_ols,
                                "Total trades": self.total_trades_figarch_oos_ols
                        }}
        ...
        def compute_errors(self, all_forecasts, all_true_values, naive_forecast):

            # turn naive forecast into a decimal for comparison
            naive_forecast_decimal = naive_forecast / (100 **2)

            # mse
            mse = np.mean((all_forecasts - all_true_values) ** 2)
            mae = np.mean(np.abs(all_forecasts - all_true_values))

            naive_mse = np.mean((naive_forecast_decimal - all_true_values) ** 2)
            naive_mae = np.mean(np.abs(naive_forecast_decimal - all_true_values))

            normalised_mse = mse / naive_mse if naive_mse != 0 else 0
            normalised_mae = mae / naive_mae if naive_mae != 0 else 0

            return normalised_mse, normalised_mae

        def get_dm_test_values(self, all_forecasts, all_forecasts_ols, true_values):
            '''
            We do a one sided DM test so that we see whether OLS has greater predictive accuracy than the baseline model.
            '''
            ...
            stat_mse, p_one_mse = dm_test(
                true_values, all_forecasts_ols, all_forecasts,
                loss=lambda u, v: (u - v) ** 2,
                h=self.H, one_sided=one_sided,
                harvey_correction=harvey,
                variance_estimator=var_est)

            stat_mae, p_one_mae = dm_test(
                true_values, all_forecasts_ols, all_forecasts,
            ...
            return stat_mse, p_one_mse, stat_mae, p_one_mae
    
        def get_statistical_error_metrics_for_all(self):

            # BMSM
            norm_mse_bmsm, norm_mae_bmsm = self.compute_errors(self.forecasts_mse_mae_bmsm, self.true_var_series, self.naive_forecast_bmsm_backtest)

            norm_mse_bmsm_ols, norm_mae_bmsm_ols = self.compute_errors(self.forecasts_mse_mae_bmsm_ols, self.true_var_series, self.naive_forecast_bmsm_backtest)

            stat_mse_bmsm, p_one_mse_bmsm, stat_mae_bmsm, p_one_mae_bmsm = self.get_dm_test_values(
                self.forecasts_mse_mae_bmsm, self.forecasts_mse_mae_bmsm_ols, self.true_var_series)

            # GARCH
            norm_mse_garch, norm_mae_garch = self.compute_errors(self.forecasts_mse_mae_garch, self.true_var_series, self.naive_forecast_garch_backtest)

            norm_mse_garch_ols, norm_mae_garch_ols = self.compute_errors(self.forecasts_mse_mae_garch_ols, self.true_var_series, self.naive_forecast_garch_backtest)

            stat_mse_garch, p_one_mse_garch, stat_mae_garch, p_one_mae_garch = self.get_dm_test_values(
                self.forecasts_mse_mae_garch, self.forecasts_mse_mae_garch_ols, self.true_var_series)

            # FIGARCH
            norm_mse_figarch, norm_mae_figarch = self.compute_errors(self.forecasts_mse_mae_figarch, self.true_var_series, self.naive_forecast_figarch_backtest)

            norm_mse_figarch_ols, norm_mae_figarch_ols = self.compute_errors(self.forecasts_mse_mae_figarch_ols, self.true_var_series, self.naive_forecast_figarch_backtest)

            stat_mse_figarch, p_one_mse_figarch, stat_mae_figarch, p_one_mae_figarch = self.get_dm_test_values(
                self.forecasts_mse_mae_figarch, self.forecasts_mse_mae_figarch_ols, self.true_var_series)

            data = {
                "Norm MSE": {
                    "BMSM": norm_mse_bmsm,
                    "BMSM OLS": norm_mse_bmsm_ols,
                    "GARCH": norm_mse_garch,
                    "GARCH OLS": norm_mse_garch_ols,
                    "FIGARCH": norm_mse_figarch,
                    "FIGARCH OLS": norm_mse_figarch_ols,
                },

                "DM Test Stat MSE": {
                    "BMSM": np.nan,
                    "BMSM OLS": stat_mse_bmsm,
                    "GARCH": np.nan,
                    "GARCH OLS": stat_mse_garch,
                    "FIGARCH": np.nan,
                    "FIGARCH OLS": stat_mse_figarch,
                }, 

                "DM p-value (one-sided) MSE":{
                    "BMSM": np.nan,
                    "BMSM OLS": p_one_mse_bmsm,
                    "GARCH": np.nan,
                    "GARCH OLS": p_one_mse_garch,
                    "FIGARCH": np.nan,
                    "FIGARCH OLS": p_one_mse_figarch,
                },

                "Norm MAE": {
                    "BMSM": norm_mae_bmsm,
                    "BMSM OLS": norm_mae_bmsm_ols,
                    "GARCH": norm_mae_garch,
                    "GARCH OLS": norm_mae_garch_ols,
                    "FIGARCH": norm_mae_figarch,
                    "FIGARCH OLS": norm_mae_figarch_ols,
                },
                "DM Test Stat MAE": {
                    "BMSM": np.nan,
                    "BMSM OLS": stat_mae_bmsm,
                    "GARCH": np.nan,
                    "GARCH OLS": stat_mae_garch,
                    "FIGARCH": np.nan,
                    "FIGARCH OLS": stat_mae_figarch,
                },
                "DM p-value (one-sided) MAE":{
                    "BMSM": np.nan,
                    "BMSM OLS": p_one_mae_bmsm,
                    "GARCH": np.nan,
                    "GARCH OLS": p_one_mae_garch,
                    "FIGARCH": np.nan,
                    "FIGARCH OLS": p_one_mae_figarch,
                }
            }
        ...
        def in_sample_predictions(self):
            '''
            Exclusively in-sample predictions for OLS regression fitting and variance forecasts.

            Collect log-likelihood, estimated params, standard errors, M-Z p-values and values, DM p-values 
            - hence need to run OLS on full forecasts but can use same model as we trained on in-sample values. 
            '''

            #BMSM
            X, Xt, b, y_log, m_log, m_logt = get_regression_features_MSM(self.forecasts_train_ols_bmsm_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_bmsm_backtest)], \
                                                                            self.stds_of_vars_bmsm_backtest_train, self.true_var_series_train_ols_bmsm_backtest, self.Pi_train_backtest,\
                                                                             self.forecasts_mse_mae_bmsm, self.mid_vol_smile_test, self.stds_vars_mse_mae_bmsm, self.Pi_test_backtest, self.bit_states_backtest, self.m0_backtest)
                
            self.forecasts_mse_mae_bmsm_ols, self.smear_bmsm, self.model_bmsm, self.scalar_bmsm = regression_forecasts_train(\
                    X, b, y_log, m_log, m_logt, Xt)
            
            # GARCH
            X, Xt, b, y_log, m_log, m_logt = get_regression_features_OTHER(self.forecasts_train_ols_garch_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_garch_backtest)], \
                                                                            self.stds_of_vars_garch_backtest_train, self.true_var_series_train_ols_garch_backtest, 
                                                                            self.forecasts_mse_mae_garch, self.mid_vol_smile_test, self.stds_vars_mse_mae_garch)

            self.forecasts_mse_mae_garch_ols, self.smear_garch, self.model_garch, self.scalar_garch = regression_forecasts_train(\
                    X, b, y_log, m_log, m_logt, Xt)
            
            # FIGARCH
            X, Xt, b, y_log, m_log, m_logt = get_regression_features_OTHER(self.forecasts_train_ols_figarch_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_figarch_backtest)], \
                                                                            self.stds_of_vars_figarch_backtest_train, self.true_var_series_train_ols_figarch_backtest, 
                                                                            self.forecasts_mse_mae_figarch, self.mid_vol_smile_test, self.stds_vars_mse_mae_figarch)

            self.forecasts_mse_mae_figarch_ols, self.smear_figarch, self.model_figarch, self.scalar_figarch = regression_forecasts_train(\
                    X, b, y_log, m_log, m_logt, Xt)

            # after this call in sample errors and collect all data
            statistical_errors_df = self.get_statistical_error_metrics_for_all()

            # collect log likelihoods
            log_ls = {
                "BMSM": self.log_L_bmsm,
                "GARCH": self.log_L_garch,
                "FIGARCH": self.log_L_figarch}

            #collect M-Z values
            m_z_results = {
                 "BMSM":
                 {'alpha_hat': self.alpha_hat_bmsm, 'beta_hat': self.beta_hat_bmsm, 'alpha_p': self.alpha_p_bmsm, 'beta_p': self.beta_p_bmsm} ,
                 "GARCH":
                 {'alpha_hat': self.alpha_hat_garch, 'beta_hat': self.beta_hat_garch, 'alpha_p': self.alpha_p_garch, 'beta_p': self.beta_p_garch} ,
                 "FIGARCH":
                 {'alpha_hat': self.alpha_hat_figarch, 'beta_hat': self.beta_hat_figarch, 'alpha_p': self.alpha_p_figarch, 'beta_p': self.beta_p_figarch}
            }
            ...
            se_results = {
                 'BMSM': self.se_bmsm,
                 'GARCH': self.se_garch,
                 'FIGARCH': self.se_figarch
            }

            return statistical_errors_df, log_ls, m_z_results, se_results