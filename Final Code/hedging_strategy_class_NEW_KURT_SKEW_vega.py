...
def run_strategy_with_params(forecasts, test_implied_vol, test_vol_smile, r_base, r_term, 
    ...
    cash, trading_portfolio_value, total_portfolio_value, trading_summary, option_delta_positions, delta_hedge, delta_hedge_cashflows, hedge_carry, daily_option_pnl_real, daily_option_pnl_mtm, daily_hedge_pnl_mtm, vega = strategy.run_strategy()
    ...
    precision_all, precision_active, win_loss_ratio_all, win_loss_ratio_active, avg_win_all, avg_win_active, avg_loss_all, avg_loss_active, non_active_share, total_trades, vega
...
class Hedging_Trading_Strategy:
    ...
    def run_strategy(self):
        ...
        if self.convert_USD:
            self.convert_to_USD()
            return self.MM_V_USD, self.trading_V_USD, self.V_USD, self.trading_summary, self.option_delta_position, self.total_hedge_position, self.adjustment_cashflow_USD, self.hedge_carry_USD, self.daily_option_pnl_real_USD, self.daily_option_pnl_mtm_USD, self.daily_hedge_pnl_mtm_USD, self.vega
        else:
            return self.MM_V, self.trading_V, self.V, self.trading_summary, self.option_delta_position, self.total_hedge_position, self.adjustment_cashflow, self.hedge_carry, self.daily_option_pnl_real, self.daily_option_pnl_mtm, self.daily_hedge_pnl_mtm, self.vega
...
class Compare_Trading_Strategies:
        ...
        def get_BMSM_data(self):
                ...
                norm_mse, norm_mae, R2, Qlike, realised_vol_H_forecasts, realised_vol_H_true, stds_of_vols, realised_vol_H_forecasts_train, realised_vol_H_true_train, stds_of_vols_train, naive_forecast = bmsm_forecasts_backtest.compute_forecasts()
                self.Pi_train_backtest, self.Pi_test_backtest, self.bit_states_backtest, self.m0_backtest = bmsm_forecasts_backtest.access_extra_values()
                ...
                self.stds_of_vols_bmsm_backtest_train = stds_of_vols_train
                self.stds_of_vols_bmsm_backtest = stds_vol_backtest
                ...
                norm_mse, norm_mae, R2, Qlike, realised_vol_H_forecasts, realised_vol_H_true, stds_of_vols, realised_vol_H_forecasts_train, realised_vol_H_true_train, stds_of_vols_train, naive_forecast = bmsm_forecasts_oos.compute_forecasts()
                self.Pi_train_oos, self.Pi_test_oos, self.bit_states_oos, self.m0_oos = bmsm_forecasts_oos.access_extra_values()
                ...
                self.true_vol_series_train_ols_bmsm_oos = realised_vol_H_true_train / 100 

                self.stds_of_vols_bmsm_oos_train = stds_of_vols_train
                self.stds_of_vols_bmsm_oos = stds_of_vols[1:]
        ...
        def run_trading_strategy(self, forecasts, signal, long_optimal, short_optimal, sig_mult, type, model, plots, verbose):
                ...
                if type == 'Backtest':
                        ...
                        avg_loss_all_bt, avg_loss_active_bt, non_active_share_bt, total_trades_bt, vega_bt = run_strategy_with_params(
                                forecasts=forecasts,
                        ...
                        calmar_bt, cagr_bt, cash_no_trades, precision_all_bt, precision_active_bt, \
                        win_loss_ratio_all_bt, win_loss_ratio_active_bt, avg_win_all_bt, avg_win_active_bt, \
                        avg_loss_all_bt, avg_loss_active_bt, non_active_share_bt, total_trades_bt, vega_bt
                        ...
                        avg_loss_all_oos, avg_loss_active_oos, non_active_share_oos, total_trades_oos, vega_oos = run_strategy_with_params(
                                forecasts=forecasts,
                        ...
                        calmar_oos, cagr_oos, cash_no_trades, precision_all_oos, precision_active_oos, \
                        win_loss_ratio_all_oos, win_loss_ratio_active_oos, avg_win_all_oos, avg_win_active_oos, \
                        avg_loss_all_oos, avg_loss_active_oos, non_active_share_oos, total_trades_oos, vega_oos
        ...
        def get_BMSM_hyperparams_and_run_strategy(self, verbose):
                ...
                self.long_optimal_bmsm, self.short_optimal_bmsm, self.sig_mult_bmsm = \
                        self.get_hyperparameters(self.forecasts_bmsm_backtest, self.signal_bmsm_backtest, self.forecasts_train_ols_bmsm_backtest, \
                                                 self.true_vol_series_train_ols_bmsm_backtest, self.stds_of_vols_bmsm_backtest_train, \
                                                 self.stds_of_vols_bmsm_backtest, MSM_indicator=True, OLS_bool=False)
                self.name_bmsm = 'BMSM'
                ...
                self.cash_bmsm_bt, self.trading_portfolio_value_bmsm_bt, self.total_portfolio_value_bmsm_bt, \
                        ...
                        self.avg_loss_all_bmsm_bt, self.avg_loss_active_bmsm_bt, self.non_active_share_bmsm_bt, self.total_trades_bmsm_bt, self.vega_bmsm_bt = \
                ...
                self.long_optimal_bmsm_ols, self.short_optimal_bmsm_ols, self.sig_mult_bmsm_ols = \
                        self.get_hyperparameters(self.forecasts_bmsm_backtest, self.signal_bmsm_backtest, self.forecasts_train_ols_bmsm_backtest, \
                                                 self.true_vol_series_train_ols_bmsm_backtest, self.stds_of_vols_bmsm_backtest_train, \
                                                 self.stds_of_vols_bmsm_backtest, MSM_indicator=True, OLS_bool=True)
                ...
                X, Xt, b, y_log, m_log, m_logt = get_regression_features_MSM(self.forecasts_train_ols_bmsm_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_bmsm_backtest)], \
                                                                            self.stds_of_vols_bmsm_backtest_train, self.true_vol_series_train_ols_bmsm_backtest, self.Pi_train_backtest,\
                                                                             self.forecasts_bmsm_backtest, self.vol_smile_backtest, self.stds_of_vols_bmsm_backtest, self.Pi_test_backtest, self.bit_states_backtest, self.m0_backtest)
                ...
                self.cash_bmsm_bt_ols, self.trading_portfolio_value_bmsm_bt_ols, self.total_portfolio_value_bmsm_bt_ols, \
                        ...
                        self.avg_loss_all_bmsm_bt_ols, self.avg_loss_active_bmsm_bt_ols, self.non_active_share_bmsm_bt_ols, self.total_trades_bmsm_bt_ols, self.vega_bmsm_bt_ols  = \
                ...
                self.cash_bmsm_oos, self.trading_portfolio_value_bmsm_oos, self.total_portfolio_value_bmsm_oos, \
                        ...
                        self.avg_loss_all_bmsm_oos, self.avg_loss_active_bmsm_oos, self.non_active_share_bmsm_oos, self.total_trades_bmsm_oos, self.vega_bmsm_oos = \
                ...
                X_oos, Xt_oos, b_oos, y_log_oos , m_log_oos, m_logt_oos = get_regression_features_MSM(self.forecasts_train_ols_bmsm_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_bmsm_oos)], \
                                                                            self.stds_of_vols_bmsm_oos_train, self.true_vol_series_train_ols_bmsm_oos, self.Pi_train_oos,\
                                                                             self.forecasts_bmsm_oos, self.vol_smile_oos, self.stds_of_vols_bmsm_oos, self.Pi_test_oos, self.bit_states_oos, self.m0_oos)
                ...
                self.cash_bmsm_oos_ols, self.trading_portfolio_value_bmsm_oos_ols, self.total_portfolio_value_bmsm_oos_ols, \
                        ...
                        self.avg_loss_all_bmsm_oos_ols, self.avg_loss_active_bmsm_oos_ols, self.non_active_share_bmsm_oos_ols, self.total_trades_bmsm_oos_ols, self.vega_bmsm_oos_ols = \
        ...
        def get_GARCH_data(self):
                ...
                if self.garch_1_2_indicator:
                        ...
                        norm_mse, norm_mae, R2, Qlike, realised_vol_H_forecasts, realised_vol_H_true, stds_of_vols, forecasts_train, true_vols_train, stds_of_vols_train, naive_forecast = garch_forecaster.forecast_garch()
                        ...
                        self.stds_of_vols_garch_backtest_train = stds_of_vols_train
                        self.stds_of_vols_garch_backtest = stds_vol_backtest
                        ...
                        res = garch_forecaster.fit_garch_model_library()
                        norm_mse, norm_mae, R2, Qlike, realised_vol_H_forecasts, realised_vol_H_true, stds_of_vols, forecasts_train, realised_vol_H_true_train, stds_of_vols_train, naive_forecast = garch_forecaster.forecast_garch()
                        ...
                        self.true_vol_series_train_ols_garch_oos = realised_vol_H_true_train / 100 

                        self.stds_of_vols_garch_oos_train = stds_of_vols_train
                        self.stds_of_vols_garch_oos = stds_of_vols[1:]
                        ...
                        norm_mse, norm_mae, R2, Qlike, realised_vol_H_forecasts, realised_vol_H_true, stds_of_vols, forecasts_train, true_train, stds_of_vols_train, naive_forecast = garch_forecaster.forecast_garch()
        ...
        def get_GARCH_hyperparams_and_run_strategy(self, verbose):
                ...
                self.long_optimal_garch, self.short_optimal_garch, self.sig_mult_garch = \
                        self.get_hyperparameters(self.forecasts_garch_backtest, self.signal_garch_backtest, self.forecasts_train_ols_garch_backtest, \
                                                 self.true_vol_series_train_ols_garch_backtest, self.stds_of_vols_garch_backtest_train, \
                                                 self.stds_of_vols_garch_backtest, MSM_indicator=False, OLS_bool=False)
                ...
                self.long_optimal_garch_ols, self.short_optimal_garch_ols, self.sig_mult_garch_ols = \
                        self.get_hyperparameters(self.forecasts_garch_backtest, self.signal_garch_backtest, self.forecasts_train_ols_garch_backtest, \
                                                 self.true_vol_series_train_ols_garch_backtest, self.stds_of_vols_garch_backtest_train, \
                                                 self.stds_of_vols_garch_backtest, MSM_indicator=False, OLS_bool=True)
                ...
                X, Xt, b, y_log, m_log, m_logt = get_regression_features_OTHER(self.forecasts_train_ols_garch_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_garch_backtest)], \
                                                                            self.stds_of_vols_garch_backtest_train, self.true_vol_series_train_ols_garch_backtest, 
                                                                            self.forecasts_garch_backtest, self.vol_smile_backtest, self.stds_of_vols_garch_backtest)
                ...
                X_oos, Xt_oos, b_oos, y_log_oos, m_log_oos, m_logt_oos = get_regression_features_OTHER(self.forecasts_train_ols_garch_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_garch_oos)], \
                                                                            self.stds_of_vols_garch_oos_train, self.true_vol_series_train_ols_garch_oos,\
                                                                             self.forecasts_garch_oos, self.vol_smile_oos, self.stds_of_vols_garch_oos)
        ...
        def get_FIGARCH_data(self):
                ...
                norm_mse, norm_mae, R2, Qlike, realised_vol_H_forecasts, realised_vol_H_true, stds_of_vols, forecasts_train, true_vols_train, stds_of_vols_train, naive_forecast = figarch_forecaster_bt.forecast_figarch()
                ...
                self.stds_of_vols_figarch_backtest_train = stds_of_vols_train
                self.stds_of_vols_figarch_backtest = stds_vol_backtest
                ...
                norm_mse, norm_mae, R2, Qlike, realised_vol_H_forecasts, realised_vol_H_true, stds_of_vols, forecasts_train, realised_vol_H_true_train, stds_of_vols_train, naive_forecast = figarch_forecaster_oos.forecast_figarch()
                ...
                self.true_vol_series_train_ols_figarch_oos = realised_vol_H_true_train / 100

                self.stds_of_vols_figarch_oos_train = stds_of_vols_train
                self.stds_of_vols_figarch_oos = stds_of_vols[1:]
        ...
        def get_FIGARCH_hyperparams_and_run_strategy(self, verbose):
            ...
            self.long_optimal_figarch, self.short_optimal_figarch, self.sig_mult_figarch = \
                        self.get_hyperparameters(self.forecasts_figarch_backtest, self.signal_figarch_backtest, self.forecasts_train_ols_figarch_backtest, \
                                                 self.true_vol_series_train_ols_figarch_backtest, self.stds_of_vols_figarch_backtest_train, \
                                                 self.stds_of_vols_figarch_backtest, MSM_indicator=False, OLS_bool=False)
            ...
            self.long_optimal_figarch_ols, self.short_optimal_figarch_ols, self.sig_mult_figarch_ols = \
                        self.get_hyperparameters(self.forecasts_figarch_backtest, self.signal_figarch_backtest, self.forecasts_train_ols_figarch_backtest, \
                                                 self.true_vol_series_train_ols_figarch_backtest, self.stds_of_vols_figarch_backtest_train, \
                                                 self.stds_of_vols_figarch_backtest, MSM_indicator=False, OLS_bool=True)
            ...
            X, Xt, b, y_log, m_log, m_logt = get_regression_features_OTHER(self.forecasts_train_ols_figarch_backtest, self.mid_vol_smile_train.iloc[:len(self.forecasts_train_ols_figarch_backtest)], \
                                                                            self.stds_of_vols_figarch_backtest_train, self.true_vol_series_train_ols_figarch_backtest, 
                                                                            self.forecasts_figarch_backtest, self.vol_smile_backtest, self.stds_of_vols_figarch_backtest)
            ...
            X_oos, Xt_oos, b_oos, y_log_oos, m_log_oos, m_logt_oos = get_regression_features_OTHER(self.forecasts_train_ols_figarch_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_figarch_oos)], \
                                                                            self.stds_of_vols_figarch_oos_train, self.true_vol_series_train_ols_figarch_oos,\
                                                                             self.forecasts_figarch_oos, self.vol_smile_oos, self.stds_of_vols_figarch_oos)
        ...
        def compare_strategies(self, verbose):
                ...
                gs = GridSpec(nrows=2*len(pairs), ncols=1, height_ratios=[4,1]*len(pairs), hspace=0.3)
                backtest_idx = len(self.forecasts_bmsm_backtest)
                first_top_ax = None
                ...
                for i, ((name_raw, fc_bt_raw, fc_oos_raw,
                    ...
                    ax.plot(self.true_vol_series*100, lw=1.5, ls='--', label='Actual Realised',
                            color=COLORS["actual"])
                    ax.plot(self.atm_mid_implied_vol_test*100, lw=1.3, ls='-', label='ATM Mid Implied',
                            color=COLORS["iv"], alpha=0.8)

                    ax.plot(np.hstack((fc_bt_raw, fc_oos_raw))*100, lw=1.2, ls='-',
                            label=f'{name_raw} Forecast', color=c_raw, alpha=0.95)
                    ax.plot(np.hstack((fc_bt_ols, fc_oos_ols))*100, lw=1.2, ls='-',
                    ...
                    ax.grid(alpha=0.35)
                    ax.set_title(f'{name_ols}: Forecasts vs Realised Volatility and Daily MtM Portfolio Returns')
                ...
                fig.suptitle(f'{self.H}-Day Annualised Volatility: OLS Correction for {self.ticker}', fontsize=18, y=0.995)
                fig.tight_layout()
        ...
        def compute_errors(self, all_forecasts, all_true_values, naive_forecast):
            ...
            naive_forecast_decimal = naive_forecast / 100
        ...
        def get_statistical_error_metrics_for_all(self):
            ...
            # BMSM
            all_forecasts_bmsm = np.hstack((self.forecasts_bmsm_backtest, self.forecasts_bmsm_oos))
            norm_mse_bmsm, norm_mae_bmsm = self.compute_errors(all_forecasts_bmsm, self.true_vol_series, self.naive_forecast_bmsm_backtest)

            all_forecasts_bmsm_ols = np.hstack((self.forecasts_bmsm_backtest_ols, self.forecasts_bmsm_oos_ols))
            norm_mse_bmsm_ols, norm_mae_bmsm_ols = self.compute_errors(all_forecasts_bmsm_ols, self.true_vol_series, self.naive_forecast_bmsm_backtest)

            stat_mse_bmsm, p_one_mse_bmsm, stat_mae_bmsm, p_one_mae_bmsm = self.get_dm_test_values(
                all_forecasts_bmsm, all_forecasts_bmsm_ols, self.true_vol_series)

            # GARCH
            all_forecasts_garch = np.hstack((self.forecasts_garch_backtest, self.forecasts_garch_oos))
            norm_mse_garch, norm_mae_garch = self.compute_errors(all_forecasts_garch, self.true_vol_series, self.naive_forecast_garch_backtest)

            all_forecasts_garch_ols = np.hstack((self.forecasts_garch_backtest_ols, self.forecasts_garch_oos_ols))
            norm_mse_garch_ols, norm_mae_garch_ols = self.compute_errors(all_forecasts_garch_ols, self.true_vol_series, self.naive_forecast_garch_backtest)

            stat_mse_garch, p_one_mse_garch, stat_mae_garch, p_one_mae_garch = self.get_dm_test_values(
                all_forecasts_garch, all_forecasts_garch_ols, self.true_vol_series)

            # FIGARCH
            all_forecasts_figarch = np.hstack((self.forecasts_figarch_backtest, self.forecasts_figarch_oos))
            norm_mse_figarch, norm_mae_figarch = self.compute_errors(all_forecasts_figarch, self.true_vol_series, self.naive_forecast_figarch_backtest)

            all_forecasts_figarch_ols = np.hstack((self.forecasts_figarch_backtest_ols, self.forecasts_figarch_oos_ols))
            norm_mse_figarch_ols, norm_mae_figarch_ols = self.compute_errors(all_forecasts_figarch_ols, self.true_vol_series, self.naive_forecast_figarch_backtest)

            stat_mse_figarch, p_one_mse_figarch, stat_mae_figarch, p_one_mae_figarch = self.get_dm_test_values(
                all_forecasts_figarch, all_forecasts_figarch_ols, self.true_vol_series)
