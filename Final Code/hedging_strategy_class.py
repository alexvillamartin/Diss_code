...
def get_regression_features_MSM(train_forecasts, train_implied_vol_smile, stds_of_vols_train, train_true_vols, \
    ...
    Put_wing_10 = train_implied_vol_smile['-0.1'] - train_implied_vol_smile['ATM']
    change_in_PW10 = Put_wing_10 - Put_wing_10.shift(1)
    change_in_PW10 = change_in_PW10.bfill() 

    BF_25 = (train_implied_vol_smile['-0.25'] + train_implied_vol_smile['0.25']) / 2 - train_implied_vol_smile['ATM']

    X = np.column_stack((x1, x2, change_in_PW10.iloc[:len(x1)], BF_25.iloc[:len(x1)]))

    x1_t = np.log(stds_of_vols_backtest)
    x2_t = slow_backtest - fast_backtest

    Put_wing_10_t = backtest_implied_vol_smile['-0.1'] - backtest_implied_vol_smile['ATM']
    change_in_PW10_t = Put_wing_10_t - Put_wing_10_t.shift(1)
    final_PW_train = (backtest_implied_vol_smile['-0.1'].iloc[0] - backtest_implied_vol_smile['ATM'].iloc[0]) - (train_implied_vol_smile['-0.1'].iloc[-1] - train_implied_vol_smile['ATM'].iloc[-1])
    change_in_PW10_t.iloc[0] = final_PW_train

    BF_25_t = (backtest_implied_vol_smile['-0.25'] + backtest_implied_vol_smile['0.25']) / 2 - backtest_implied_vol_smile['ATM']

    X_t = np.column_stack((x1_t, x2_t, change_in_PW10_t.iloc[:len(x1_t)], BF_25_t.iloc[:len(x1_t)]))
...
def get_regression_features_OTHER(train_forecasts, train_implied_vol_smile, stds_of_vols_train, train_true_vols, \
    ...
    X = np.column_stack((x1, change_in_PW10.iloc[:len(x1)], BF_25.iloc[:len(x1)]))
    ...
    X_t = np.column_stack((x1_t, change_in_PW10_t.iloc[:len(x1_t)], BF_25_t.iloc[:len(x1_t)]))
...
def regression_forecasts_ridge_train(X, b, alpha, y_log, m_log, m_log_t, X_t):
    '''
...
def regression_forecasts_ridge_test(X_t, m_log_t, smear, beta, scalar):
...
def grid_search_params(backtest_data, param_grid, init_cap, OPTION_MATURITY, SIGNAL_LB, SIGNAL_UB, 
    ...
    alphas = tuple(float(x) for x in param_grid['alpha']) if OLS else (0.0,)
    ...
    def run_once(long_thresh, short_thresh, signal_multiplier, alpha):

        if OLS:
            fc_use, _, _, _ = regression_forecasts_ridge_train(
                X, b, alpha,
                y_log, m_log, m_log_t, X_t
            )
        else:
            fc_use = forecasts
    ...
    def objective(trial: optuna.Trial):
        ...
        sharpe, max_dd, calmar, cagr = \
                    run_once(long_thresh, short_thresh, signal_multiplier, alpha)
...
class Compare_Trading_Strategies:
        ...
        def get_hyperparameters(self, forecasts_backtest, signal_backtest, forecasts_train_ols, true_train_ols, train_stds, test_stds, MSM_indicator, OLS_bool):
                ...
                alpha = best_results['alpha'].iloc[0]

                return long_optimal, short_optimal, sig_mult, alpha
        ...
        def get_BMSM_hyperparams_and_run_strategy(self, verbose):
                ...
                self.long_optimal_bmsm, self.short_optimal_bmsm, self.sig_mult_bmsm, self.alpha_bmsm = \
                ...
                self.long_optimal_bmsm_ols, self.short_optimal_bmsm_ols, self.sig_mult_bmsm_ols, self.alpha_bmsm_ols = \
                        self.get_hyperparameters(self.forecasts_bmsm_backtest, self.signal_bmsm_backtest, self.forecasts_train_ols_bmsm_backtest, \
                                                 self.true_vol_series_train_ols_bmsm_backtest, self.stds_of_vols_bmsm_backtest_train, \
                ...
                self.forecasts_bmsm_backtest_ols, self.smear_bmsm, self.beta_bmsm, self.scalar_bmsm = regression_forecasts_ridge_train(\
                    X, b, self.alpha_bmsm_ols, y_log, m_log, m_logt, Xt)

                self.name_bmsm_ols = "BMSM with Ridge"
                ...
                _, Xt_oos, b_, _, _, m_logt_oos = get_regression_features_MSM(self.forecasts_train_ols_bmsm_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_bmsm_oos)], \
                ...
                self.forecasts_bmsm_oos_ols = regression_forecasts_ridge_test(Xt_oos, m_logt_oos, self.smear_bmsm, self.beta_bmsm, self.scalar_bmsm)
        ...
        def get_GARCH_hyperparams_and_run_strategy(self, verbose):
                ...
                self.long_optimal_garch, self.short_optimal_garch, self.sig_mult_garch, self.alpha_garch = \
                ...
                # do with OLS to compare without having to re-run estimations
                self.long_optimal_garch_ols, self.short_optimal_garch_ols, self.sig_mult_garch_ols, self.alpha_garch_ols = \
                        self.get_hyperparameters(self.forecasts_garch_backtest, self.signal_garch_backtest, self.forecasts_train_ols_garch_backtest, \
                                                 self.true_vol_series_train_ols_garch_backtest, self.stds_of_vols_garch_backtest_train, \
                ...
                self.forecasts_garch_backtest_ols, self.smear_garch, self.beta_garch, self.scalar_garch = regression_forecasts_ridge_train(\
                    X, b, self.alpha_garch_ols, y_log, m_log, m_logt, Xt)

                self.name_garch_ols = "GARCH with Ridge"
                ...
                _, Xt_oos, b_, _, _, m_logt_oos = get_regression_features_OTHER(self.forecasts_train_ols_garch_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_garch_oos)], \
                ...
                self.forecasts_garch_oos_ols = regression_forecasts_ridge_test(Xt_oos, m_logt_oos, self.smear_garch, self.beta_garch, self.scalar_garch)                
        ...
        def get_FIGARCH_hyperparams_and_run_strategy(self, verbose):
            ...
            # do strat wihtout OLS
            self.long_optimal_figarch, self.short_optimal_figarch, self.sig_mult_figarch, self.alpha_figarch = \
            ...
            # do with OLS to compare without having to re-run estimations
            self.long_optimal_figarch_ols, self.short_optimal_figarch_ols, self.sig_mult_figarch_ols, self.alpha_figarch_ols = \
                        self.get_hyperparameters(self.forecasts_figarch_backtest, self.signal_figarch_backtest, self.forecasts_train_ols_figarch_backtest, \
                                                 self.true_vol_series_train_ols_figarch_backtest, self.stds_of_vols_figarch_backtest_train, \
            ...
            self.forecasts_figarch_backtest_ols, self.smear_figarch, self.beta_figarch, self.scalar_figarch = regression_forecasts_ridge_train(\
                    X, b, self.alpha_figarch_ols, y_log, m_log, m_logt, Xt)

            self.name_figarch_ols = "FIGARCH with Ridge"
            ...
            _, Xt_oos, b_, _, _, m_logt_oos = get_regression_features_OTHER(self.forecasts_train_ols_figarch_oos, self.mid_vol_smile.iloc[:len(self.forecasts_train_ols_figarch_oos)], \
            ...
            self.forecasts_figarch_oos_ols = regression_forecasts_ridge_test(Xt_oos, m_logt_oos, self.smear_figarch, self.beta_figarch, self.scalar_figarch)                
        ...
        def compare_strategies(self, verbose):
                ...
                plt.plot(self.total_portfolio_value_bmsm_oos, label=f'{self.name_bmsm} Total Portfolio Value', color="#2ca02c")
                plt.plot(self.total_portfolio_value_bmsm_oos_ols, label=f'{self.name_bmsm_ols} Total Portfolio Value', color="#98df8a")
                plt.plot(self.total_portfolio_value_garch_oos, label=f'{self.name_garch} Total Portfolio Value', color="#1f77b4", linestyle = ":")
                plt.plot(self.total_portfolio_value_garch_oos_ols, label=f'{self.name_garch_ols} Total Portfolio Value', color="#aec7e8", linestyle = ":")
                plt.plot(self.total_portfolio_value_figarch_oos, label=f'{self.name_figarch} Total Portfolio Value', color="#d62728", linestyle='--')
                plt.plot(self.total_portfolio_value_figarch_oos_ols, label=f'{self.name_figarch_ols} Total Portfolio Value', color="#ff9896", linestyle='--')
                plt.xlabel("Day", fontsize=16)
                plt.ylabel("Total Portfolio Value", fontsize=16)
                ...
                plt.figure(figsize=(8, 6))
                plt.plot(self.total_portfolio_value_bmsm_bt, label=f'{self.name_bmsm} Total Portfolio Value', color="#2ca02c")
                plt.plot(self.total_portfolio_value_bmsm_bt_ols, label=f'{self.name_bmsm_ols} Total Portfolio Value', color="#98df8a")
                plt.plot(self.total_portfolio_value_garch_bt, label=f'{self.name_garch} Total Portfolio Value', color="#1f77b4", linestyle = ":")
                plt.plot(self.total_portfolio_value_garch_bt_ols, label=f'{self.name_garch_ols} Total Portfolio Value', color="#aec7e8", linestyle = ":")
                plt.plot(self.total_portfolio_value_figarch_bt, label=f'{self.name_figarch} Total Portfolio Value', color="#d62728", linestyle='--')
                plt.plot(self.total_portfolio_value_figarch_bt_ols, label=f'{self.name_figarch_ols} Total Portfolio Value', color="#ff9896", linestyle='--')
                plt.xlabel("Day", fontsize=16)
                plt.ylabel("Total Portfolio Value", fontsize=16)
                ...
                for ax, (name_raw, fc_bt_raw, fc_oos_raw,
                        name_ols, fc_bt_ols, fc_oos_ols, (c_raw, c_ols)) in zip(axs, pairs):

                    # actual + iv
                    ax.plot(self.true_vol_series * 100, lw=1.4, ls=':', label='Actual Realised', color='b')
                    ax.plot(self.atm_mid_implied_vol_test * 100, lw=1, ls='--', label='ATM Mid Implied', color='black', alpha=0.8)
