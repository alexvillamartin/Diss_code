...
def MFDFA(timeseries: np.ndarray, lag: np.ndarray, order: int = 1,
    ...
    for i in lag:
        ...
        F_positive = F[F > 0] # filter out F2 (variances) that are zero manually to prevent log0.
        f_temp = np.where(q == 0, 
                          np.exp(0.5 * np.mean(np.log(F_positive))),
                          np.float_power(
                              np.mean(np.float_power(F, q_safe / 2)),
                              1 / q_safe # q is a scalar
                          ))
        f = np.append(f, f_temp.T, axis=0)

        # Calculate standard deviation associated with each mean
        if stat is True:
            f_std = np.append(f_std,
                              np.float_power(
                                  np.std(np.float_power(F, q / 2)),
                                  1 / q.T
                              ),
                              axis=0
                              )
